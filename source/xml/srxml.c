...
static MXML_STATUS mxml_attribute_read(MXML_REFIL *data, PHB_ITEM doc, PHB_ITEM pNode,
                                       PHBXML_ATTRIBUTE dest, int32_t style);
...
static void hbxml_set_doc_status(MXML_REFIL *ref, PHB_ITEM doc, PHB_ITEM pNode, int32_t status,
                                 int32_t error)
{
...
static MXML_STATUS mxml_attribute_read(MXML_REFIL *ref, PHB_ITEM pDoc, PHB_ITEM pNode,
                                       PHBXML_ATTRIBUTE pDest, int32_t style)
{
  int32_t quotechr = '"';
  MXML_SGS *buf_name;
  MXML_SGS *buf_attrib;
...
static void mxml_node_read_cdata(MXML_REFIL *ref, PHB_ITEM pNode, PHB_ITEM pDoc)
{
  ...
  if (iStatus == 100) {
    ...
    int32_t iPos = 0, iAllocated = MXML_ALLOC_BLOCK;
...
HB_FUNC(SR_XML_DATAREAD)
{
  ...
  int32_t iStyle = hb_parni(3);
  PHB_ITEM pRoot;
  MXML_REFIL refil;
  char buffer[512], *buf;
  HB_SIZE nLen;

  if (pDoc == SR_NULLPTR || pParam == SR_NULLPTR ||
