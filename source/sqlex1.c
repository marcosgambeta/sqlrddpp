...
static HB_ERRCODE getMissingColumn(SQLEXAREAP thiswa, PHB_ITEM pFieldData, HB_LONG lFieldPosDB)
{
  ...
  if (CHECK_SQL_N_OK(res))
  {
    odbcErrorDiagRTE(thiswa->colStmt[lFieldPosDB - 1], "getMissingColumn/SQLExecute", sSql, (SQLRETURN)res, __LINE__,
                     __FILE__);
  ...
  if (res != SQL_SUCCESS)
  {
    ...
    {
      odbcErrorDiagRTE(thiswa->colStmt[lFieldPosDB - 1], "getMissingColumn/SQLFetch", sSql, (SQLRETURN)res, __LINE__,
                       __FILE__);
      SQLFreeStmt(thiswa->colStmt[lFieldPosDB - 1], SQL_CLOSE);
      return HB_FAILURE;
    }
  ...
  odbcGetData((HSTMT)thiswa->colStmt[lFieldPosDB - 1], hb_arrayGetItemPtr(thiswa->aFields, lFieldPosDB), pFieldData, 0,
  ...
  for (iRow = 1; iRow <= pageReadSize; iRow++)
  {
    ...
    for (i = 1; i <= thiswa->area.uiFieldCount; i++)
    {
      ...
      else
      {
        ...
        // FIELD_DOMAIN);
        ++iIndex;
        odbcGetData((HSTMT)thiswa->hStmtBuffer, hb_arrayGetItemPtr(thiswa->aFields, thiswa->uiBufferIndex[i - 1]), temp,
...
static HB_ERRCODE sqlExGoTop(SQLEXAREAP thiswa)
{
  ...
  if (getWorkareaParams(thiswa) == HB_FAILURE)
...
static HB_ERRCODE sqlExSeek(SQLEXAREAP thiswa, HB_BOOL bSoftSeek, PHB_ITEM pKey, HB_BOOL bFindLast)
{
  ...
  if (getPreparedSeek(thiswa, queryLevel, &iIndex, &hStmt) == HB_SUCCESS)
  { // Fetch line from database, read RECNO and DELETED
    ...
    for (i = 1; i <= thiswa->area.uiFieldCount; i++)
    {
      ...
      else
      {
        ...
        // FIELD_DOMAIN);
        ++iIndex;
        odbcGetData((HSTMT)hStmt, hb_arrayGetItemPtr(thiswa->aFields, thiswa->uiBufferIndex[i - 1]), temp, 0,
...
{
  if (thiswa->bufferHot)
  { // && (!(thiswa->ulhDeleted > 0 ? HB_TRUE : thiswa->deletedList[thiswa->recordListPos] == ' ') )
    if (thiswa->bHistoric)
    {
    ...
    if (thiswa->bIsInsert)
    {
      ...
      if (FeedRecordCols(thiswa, HB_FALSE) == HB_FAILURE)
...
HB_INIT_SYMBOLS_BEGIN(sqlEx1__InitSymbols){"SQLEX", {HB_FS_PUBLIC | HB_FS_LOCAL}, {HB_FUNCNAME(SQLEX)}, NULL},
    {"SQLEX_GETFUNCTABLE",
     {HB_FS_PUBLIC | HB_FS_LOCAL},
     {HB_FUNCNAME(SQLEX_GETFUNCTABLE)},
     NULL} HB_INIT_SYMBOLS_END(sqlEx1__InitSymbols)

        HB_CALL_ON_STARTUP_BEGIN(_hb_sqlEx_rdd_init_) hb_vmAtInit(hb_sqlExRddInit, NULL);
